...
EMCC_DEBUG=1 emcc -v -o build/libgame.so $CFLAGS -Iext -Isrc -O1 \
-std=gnu23  src/game/*.c src/gui/*.c -fPIC -shared \
...
EMCC_DEBUG=1 emcc -v -o build/index.html $CFLAGS -Iext -Isrc -Lbuild -O1 \
-std=gnu23  src/base/*.c src/core/*.c -lgame -Wl,-rpath='build/' \
-s USE_SDL=3 -s DISABLE_EXCEPTION_CATCHING=1 -s WASM=1 \
-s ALLOW_MEMORY_GROWTH=1 -s NO_EXIT_RUNTIME=0 -s ASSERTIONS=1 \
